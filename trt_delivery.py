...
from utils.yolo_classes_delivery import get_cls_dict #delivery 6classes
...
def parse_args():
    ...
    parser.add_argument(
        '-c', '--category_num', type=int, default=6, # delivery has 6 classes
...
def loop_and_detect(pub, pub2, cam, trt_yolo, conf_th, vis):
    ...
    """
    r = rospy.Rate(20) #to downgrade FPS
    full_scrn = False
    fps = 0.0
    tic = time.time()
    ...
    while True:
        ...
        for i in range(len(boxes)):
            ...
            print("class num:",obj_info.id)
           # print(sys.getsizeof(detection))
        # for i in range(len(detection2d)):
        #     print(detection2d.detections[i].ObjectHypothesisWithPose.id)
...
def main(publisher, publisher2):
    ...
    open_window(
        WINDOW_NAME, 'Delivery Mission YOLOv4',
...
if __name__ == '__main__':
    rospy.init_node('Mission_Sign', anonymous=False) # create node
    pub = rospy.Publisher('/sign_bbox', Detection2DArray , queue_size=1) # create bbox msg 
